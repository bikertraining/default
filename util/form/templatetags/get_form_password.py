...
@register.simple_tag
def get_form_password(label, name, error_text=None, help_text=None, maxlength='256', required=True, value=None):
    if error_text is None:
        error_text = {}

    context = (f'<div class="TextInput{" required" if required else ""}">'
               f'<label class="fw-bold form-label" for={name}>{label}</label>'
               f'<input class="form-control{" is-invalid" if error_text else ""}" id="{name}" name="{name}" maxlength="{maxlength}" placeholder="{label}" type="password" value="{value}">')
