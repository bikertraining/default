...
@register.simple_tag
def get_form_textarea(label, name, error_text=None, help_text=None, required=True, rows='3', maxlength='255', value=None):
    if error_text is None:
        error_text = {}

    context = (f'<div class="TextInput{" required" if required else ""}">'
               f'<label class="fw-bold form-label" for={name}>{label}</label>'
               f'<textarea class="form-control{" is-invalid" if error_text else ""}" id="{name}" name="{name}" maxlength="{maxlength}" placeholder="{label}" rows="{rows}">{value}</textarea>')
