...
@register.simple_tag
def get_form_text(label, name, error_text=None, help_text=None, is_cc=False, maxlength='256', required=True,
                  value=None):
    ...
    context = (f'<div class="TextInput{" required" if required else ""}">'
               ...
               f'<input class="form-control{" is-invalid" if error_text else ""}" id="{name}" name="{name}" maxlength="{maxlength}" placeholder="{label}" type="text" value="{value}">')
